function options() {
	...
	options_menu.querySelector("#btn-key_abilitiesHelp").innerHTML = String.fromCharCode(Game.options.controls[4]);
	...
	document.querySelector("#btn-key_abilitiesHelp").click = function(e) {changeKey(4, this)};
	...
	window.onkeydown = function(e) {
		switch (e.keyCode) {
			case 27: // echap
				/*if (Game.UI.section.options.style.display === "block" && Game.UI.wrapper.options.style.display === "none") {
					hide(Game.UI.section.options_content);
					hide(Game.UI.section.options)
				} else if (Game.UI.wrapper.options.style.display === "block") hide(Game.UI.wrapper.options);
				break*/
