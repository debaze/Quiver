function init(isLoaded) {
	...
	player.enableMoving = function() {
		...
		function move() {
			if (moveTo.top) {
				if (y >= -0.1 && x >= -1 && x <= 10) {
					select("debug #debug3").innerHTML = "touche le bord"
				} else {
					select("debug #debug3").innerHTML = "";
					y += speed
				}
			}
			...
			select("debug #debug1").innerHTML = "pos.x = " + player.pos[0];
			select("debug #debug2").innerHTML = "pos.y = " + player.pos[1];
